# Vẽ đồ thị kết hợp - Food Price in Turkey
import pandas as pd
import numpy as np
...
#1.Vẽ các biểu đồ cột so sánh giá Milk (powder, infant formula) và Fuel (gas)
# tháng 12 cuối năm năm 2019 của Ankara, Istanbul, Izmir và National Average.
d11 = df[(df['Year'] == 2019) & (df['Month'] == 12) 
        & (df['ProductName'] == 'Milk (powder, infant formula) - Retail')].reset_index()

d12 = df[(df['Year'] == 2019) & (df['Month'] == 12) 
    & (df['ProductName'] == 'Fuel (gas) - Retail')].reset_index()
...
#2.Vẽ các biểu đồ đường phân tích xu hướng giá gạo (Rice-Retail),giá Fuel (gas)
#trung bình cả nước (National Average) trong năm 2016, 2018, 2019 tại Thổ Nhĩ Kì.
